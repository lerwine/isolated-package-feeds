...
public class MainService : BackgroundService
{
    ...
    protected async override Task ExecuteAsync(CancellationToken stoppingToken)
    {
        try
        {
            ...
            if (appSettings.Validated.Import is not null)
                throw new NotImplementedException();
            // await ImportAsync(appSettings.Validated.ImportPath, localClientService, _logger, stoppingToken);
            ...
            if (appSettings.ListLocal)
                await ListLocalPackagesAsync(localClientService.GetAllPackagesAsync(stoppingToken).ToBlockingEnumerable(stoppingToken), appSettings.Validated.ExportLocalManifest?.FullName, _logger, stoppingToken);
            if (appSettings.Validated.ExportLocalManifest is not null)
                await ExportLocalManifestAsync(localClientService.GetAllPackagesAsync(stoppingToken).ToBlockingEnumerable(stoppingToken), appSettings.Validated.ExportLocalManifest.FullName, _logger, stoppingToken);
            if (appSettings.Validated.ExportBundle is not null)
                throw new NotImplementedException();
            // await ExportBundleAsync(appSettings.Validated.ExportBundlePath, appSettings.Validated.TargetManifestFilePath, appSettings.Validated.TargetManifestSaveAsPath, localClientService, _logger, stoppingToken);
        }
