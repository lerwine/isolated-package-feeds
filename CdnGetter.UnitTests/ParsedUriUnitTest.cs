...
public class ParsedUriUnitTest
{
    ...
    */
    /// <summary>
    /// Unit test for constructor <see cref="ParsedUri.ValidIpV6Regex" /> that will not throw an excePATCHaion.
    /// </summary>
    [Theory]
    ...
    public void ValidIpV6RegexTest(string address, bool embeddedIpV4, bool isBare, bool expected, string description)
    {
        ...
        if (isBare)
        {
            if (embeddedIpV4)
            {
                actual = ParsedUri.ValidIpV6wV4Regex.IsMatch(address);
                name = nameof(ParsedUri.ValidIpV6wV4Regex);
            }
            else
            {
                actual = ParsedUri.ValidIpV6Regex.IsMatch(address);
                name = nameof(ParsedUri.ValidIpV6Regex);
            }
        }
        else if (embeddedIpV4)
        {
            actual = ParsedUri.ValidUriIpV6wV4Regex.IsMatch(address);
            name = nameof(ParsedUri.ValidUriIpV6wV4Regex);
        }
        else
        {
            actual = ParsedUri.ValidUriIpV6Regex.IsMatch(address);
            name = nameof(ParsedUri.ValidUriIpV6Regex);
        }
        if (expected)
            Assert.True(actual, $"{description}: ParsedUri.{name}.IsMatch(\"{address}\")");
        else
            Assert.False(actual, $"{description}: ParsedUri.{name}.IsMatch(\"{address}\")");
        if (isBare)
        {
            if (embeddedIpV4)
            {
                actual = ParsedUri.ValidIpV6Regex.IsMatch(address);
                Assert.False(actual, $"{description}: ParsedUri.{nameof(ParsedUri.ValidIpV6Regex)}.IsMatch(\"{address}\")");
                actual = ParsedUri.ValidUriIpV6wV4Regex.IsMatch(address);
                Assert.False(actual, $"{description}: ParsedUri.{nameof(ParsedUri.ValidUriIpV6wV4Regex)}.IsMatch(\"{address}\")");
                actual = ParsedUri.ValidUriIpV6Regex.IsMatch(address);
                name = nameof(ParsedUri.ValidUriIpV6Regex);
            }
            else
            {
                actual = ParsedUri.ValidIpV6wV4Regex.IsMatch(address);
                Assert.False(actual, $"{description}: ParsedUri.{nameof(ParsedUri.ValidIpV6wV4Regex)}.IsMatch(\"{address}\")");
                actual = ParsedUri.ValidUriIpV6wV4Regex.IsMatch(address);
                Assert.False(actual, $"{description}: ParsedUri.{nameof(ParsedUri.ValidUriIpV6wV4Regex)}.IsMatch(\"{address}\")");
                actual = ParsedUri.ValidUriIpV6Regex.IsMatch(address);
                name = nameof(ParsedUri.ValidUriIpV6Regex);
    ...
    /// <summary>
    /// Unit test for constructor <see cref="ParsedUri.ValidIpV4Regex" /> that will not throw an excePATCHaion.
    /// </summary>
    [Theory]
    [ClassData(typeof(ValidIpV4RegexTestData))]
    public void ValidIpV4RegexTest(string address, bool expected)
    {
        bool actual = ParsedUri.ValidIpV4Regex.IsMatch(address);
    ...
    /// Generates test data for <see cref="ValidIpV4RegexTestData(string, bool)" />.
    /// </summary>
    public class ParsedUriParseComponentsRegexTestData : TheoryData<string, bool>
    {
        public ParsedUriParseComponentsRegexTestData()
        {
            Random r = new();
            for (int i = 0; i < 256; i++)
            {
    ...
    /// <summary>
    /// Unit test for constructor <see cref="ParsedUri.ParsedUriParseComponentsRegex" /> that will not throw an excePATCHaion.
    /// </summary>
    [Theory]
    [ClassData(typeof(ParsedUriParseComponentsRegexTestData))]
    public void ParsedUriParseComponentsRegexTest(string address, bool expected)
    {
        bool actual = ParsedUri.ValidIpV4Regex.IsMatch(address);
        Assert.Equal(expected, actual);
    }
    ...
    {
        ParsedUri.UserInfo target = new(userName, password);
