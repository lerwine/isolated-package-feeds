...
public partial class Url : IEquatable<Url>, IComparable<Url>
{
    ...
    public static readonly Regex ValidIpV6Regex = new(@"^\[((?=(([^:]+:)+((:[^:]+)+|:)|:(:[^:]+)+)$)(([\da-f]{1,4}::?){1,6}([\da-f]{1,4}(::)?)?|(::?[\da-f]{1,4}){1,7})|([\da-f]{1,4}:){7}[\da-f]{1,4}|::)\]$", RegexOptions.IgnoreCase | RegexOptions.Compiled);
    public static readonly Regex ValidIpV6wV4Regex = new(@"^\[(:(:[\da-f]{1,4}){0,5}:|[\da-f]{1,4}:(:([\da-f]{1,4}:){0,4}|[\da-f]{1,4}:(:([\da-f]{1,4}:){0,3}|[\da-f]{1,4}:(:([\da-f]{1,4}:){0,2}|[\da-f]{1,4}:(:([\da-f]{1,4}:)?|[\da-f]{1,4}:([\da-f]{1,4}?)?:)))))(([01](\d\d?)?|2([0-4]\d?|5[0-5]?|[6-9])?|[3-9]\d?)\.){3}([01](\d\d?)?|2([0-4]\d?|5[0-5]?|[6-9])?|[3-9]\d?)\]$", RegexOptions.IgnoreCase | RegexOptions.Compiled);
