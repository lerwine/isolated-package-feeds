...
namespace CdnGetter.Versioning
{
    ...
    {
        public TinyVersionNumber(byte value, bool isNegative, int zeroPadCount, ITextComponent? suffix)
        {
            if (zeroPadCount < 0)
        ...
        public TinyVersionNumber(byte value, bool isNegative, int zeroPadCount, string suffix) : this(value, isNegative, zeroPadCount, string.IsNullOrEmpty(suffix) ? null : (suffix.Length > 1) ? new StringComponent(suffix) : new CharacterComponent(suffix[0])) { }
        
        public TinyVersionNumber(byte value, bool isNegative, int zeroPadCount, char suffix) : this(value, isNegative, zeroPadCount, new CharacterComponent(suffix)) { }
        
        public TinyVersionNumber(byte value, bool isNegative, ITextComponent? suffix) : this(value, isNegative, 0, suffix) { }
        
        public TinyVersionNumber(byte value, bool isNegative, string suffix) : this(value, isNegative, 0, suffix) { }
        
        public TinyVersionNumber(byte value, bool isNegative, char suffix) : this(value, isNegative, 0, suffix) { }
        
        public TinyVersionNumber(byte value, int zeroPadCount, ITextComponent? suffix) : this(value, false, zeroPadCount, suffix) { }
        
        public TinyVersionNumber(byte value, int zeroPadCount, string suffix) : this(value, false, zeroPadCount, suffix) { }
        
        public TinyVersionNumber(byte value, int zeroPadCount, char suffix) : this(value, false, zeroPadCount, suffix) { }
        
        public TinyVersionNumber(sbyte value, int zeroPadCount, ITextComponent? suffix)
        {
            if (zeroPadCount < 0)
                throw new ArgumentOutOfRangeException(nameof(zeroPadCount));
            if (suffix is null || suffix.Length == 0)
        ...
        public TinyVersionNumber(sbyte value, int zeroPadCount, string suffix) : this(value, zeroPadCount, string.IsNullOrEmpty(suffix) ? null : (suffix.Length > 1) ? new StringComponent(suffix) : new CharacterComponent(suffix[0])) { }
        
        public TinyVersionNumber(sbyte value, int zeroPadCount, char suffix) : this(value, zeroPadCount, new CharacterComponent(suffix)) { }
