...
#pragma warning restore CA2231
{
    ...
    static Numerical32BitToken() => MAX_STRING_LENGTH = uint.MaxValue.ToString().Length;
        ...
        if (other is INumericalToken n)
        {
            if (n.IsZero)
            {
                if (Value != 0)
                    ...
                    (n is Numerical16BitToken t16) ? Value.Equals(t16.Value) : (n is Numerical32BitToken t32) ? Value.Equals(t32.Value) : (n is Numerical64BitToken t64) ?
                        (t64.Value <= uint.MaxValue && Value.Equals((uint)t64.Value)) : n.AsUnsignedBigInteger().Equals(Value)))
                return false;
        }
