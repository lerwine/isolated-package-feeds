...
namespace CdnSync
{
    public class SwVersion : IEquatable<SwVersion>, IComparable<SwVersion>
    {
        ...
        private SwVersion(Match match)
        {
            ...
            else if ((mg = match.Groups[GROUP_NAME_revision]).Success)
            {
                Revision = int.Parse(mg.Value);
                Patch = int.Parse(match.Groups[GROUP_NAME_patch].Value);
                Minor = int.Parse(match.Groups[GROUP_NAME_minor].Value);
            }
            else if ((mg = match.Groups[GROUP_NAME_patch]).Success)
            {
                Patch = int.Parse(mg.Value);
                Minor = int.Parse(match.Groups[GROUP_NAME_minor].Value);
            }
            else if ((mg = match.Groups[GROUP_NAME_minor]).Success)
